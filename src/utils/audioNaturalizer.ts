 ...
 *
 * Copied from solar-punk src/utils/audioNaturalizer.ts with one addition for
 * project-aibo: an AnalyserNode tapped off the master gain, exposed via
 * getLevel(), so VrmViewer can drive the avatar's mouth from the actual
 * audio amplitude (Phase 3 lip sync) instead of a text-length sine wave.
 *
...
export interface Naturalizer {
    ...
    onEnd: (cb: () => void) => void;
    /** RMS amplitude (0..~0.4) of the currently playing audio; 0 when idle. */
    getLevel: () => number;
}
...
export function createNaturalizer(): Naturalizer {
    ...
    function buildChain(c: AudioContext): AudioNode {
        ...
        // Lip-sync tap: master -> analyser -> destination.
    ...
    function getLevel(): number {
        if (!analyser || !levelBuf || !isPlaying) return 0;
