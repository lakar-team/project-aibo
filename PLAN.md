...
**Do:**
- [x] Identity: `localStorage` UUID `aibo:visitor` sent as `visitorId` with every request. Owner mode: typing the secret phrase (matches `OWNER_KEY` env, e.g. spoken "the witch's true name is …") flags the visitor record as Adam; his memory is permanent, visitors' expire after 30 days (Redis TTL). *(The phrase is stripped from the LLM context so it never reaches journals; owners tracked in an `aibo:owners` set for the cron.)*
- [x] Redis schema (per visitor): `aibo:mem:{id}:digest` (string ≤ 2000 tokens — the loaded memory), `aibo:mem:{id}:journal` (list — raw `memorable` lines + one-line exchange summaries, appended live), `aibo:mem:{id}:meta` (name, language prefs, last visit, owner flag). *(Plus `aibo:goals:{id}` — see goals extension below.)*
- [x] Brain route: load `digest` + `meta` into the system prompt every request ("What you remember about this visitor: …"); after each reply, if `memorable` non-null, `RPUSH` journal.
- [x] `/api/sleep`: Tier-1 model reads journal + old digest → rewrites digest (merge, dedupe, correct, prune to budget) → clears journal → stores a one-line "dream". Triggers: (a) reflex "goodnight" → run + she says a Kip-style goodnight line; (b) `navigator.sendBeacon('/api/sleep?visitor=…')` on `visibilitychange→hidden` if journal non-empty; (c) `vercel.json` cron `0 15 * * *` hitting `/api/sleep?all=owner` (cron secret header `CRON_SECRET`). *(Deterministic fallback merge when no LLM is reachable; goodnight trigger rides the new `reflexKind` done-frame field, fired client-side.)*
- [x] Morning greeting: if owner + last visit yesterday+, greeting references the digest and the stored dream ("I dreamt about that folder we cleaned up" pattern — see Kip PERSONALITY.md sample lines). *(Implemented as prompt injection when owner + >12h gap; untestable until OWNER_KEY is set.)*
- [x] **Extension (Adam, 2026-07-07): self-amending goals.** New `"goal"` field in the brain contract — lasting behavioral instructions ("from now on…", "be more…") are rewritten by the LLM into short imperative directives, acknowledged as a personal commitment, `RPUSH`ed live to `aibo:goals:{id}` (cap 20), injected into every system prompt, and consolidated during sleep (dedupe, newer-wins conflicts, ≤10).
...
**Current phase:** Phase 7 ✅ complete (2026-07-07) → next is **Phase 8**
**Last session:** 2026-07-07, Claude Fable 5 — Phase 7 (sleeping memory + Adam's self-amending-goals extension) shipped, commit `527a891`.
Phase 7 live verification (prod KV): "My name is Kenji, I raise koi in Niigata" → `memorable` journaled; "From now on, be more poetic" → `goal:"Be more poetic in responses"` with a personal-commitment acknowledgment; a FRESH conversation lists the goal and already speaks poetically (goal injection works); `/api/sleep` → `{consolidated, dream:"Crimson scales shimmered in a moonlit pond…"}`; new conversation recalls "Kenji! The keeper of splendid koi in Niigata" (digest recall); a different visitorId gets no leaked facts (isolation holds — though she role-plays vague familiarity with strangers; minor character quirk, Phase 8 material).
**Still untestable until Adam sets env vars:** owner mode + morning dream greeting (`OWNER_KEY`), cron consolidation (`CRON_SECRET`; the cron entry is live in vercel.json — Vercel sends the bearer automatically once the env exists).
Post-Phase-6 fixes, commit `52cb754`: (1) **Companion-not-promoter** — `adamProfile.ts` replaced (solar-punk CV narrative + portfolio sitemap removed from the prompt entirely; she now only knows Adam is her creator/trusted friend and must not volunteer his work); personality + greeting instruction aligned. Verified live: tired-after-work small talk gets a companion reply, no portfolio. (2) **Vision switched to OpenRouter** (`VISION_MODEL` env, default `meta-llama/llama-3.2-11b-vision-instruct`; OpenAI-style multimodal content array; `streamGemini` back to text-only). **SIGHT NOW WORKS LIVE** — test JPEG described correctly ("a blank white square") with the existing OPENROUTER_API_KEY; no Gemini key needed for vision anymore. (3) Fix-2 audit: Phase 5 commit `529281f` confirmed on origin/main; setEmotion/playGesture/setAvatarState wiring confirmed consumed in page.tsx; no bug found.
...
Phase 6 verification: browser — "how do I look today?" → consent dialog → (no camera in test browser) graceful spoken apology with no brain call; eyes-off → vision phrase goes straight to the brain, no dialog/camera. Live — after the `52cb754` provider switch, a real JPEG is described correctly by `meta-llama/llama-3.2-11b-vision-instruct` via OpenRouter; blind fallback chain retained; ordinary messages untouched (reflex still fires). Adam should try "how do I look?" with a real camera for the full experience.
